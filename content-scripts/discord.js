 ...
 * Monitors Discord messages, detects images, and uses multimodal AI for analysis
 */
...
(function() {
  ...
  async function initialize() {
    ...
    console.log('[GhostHub] Initializing Discord integration...');

    try {
      await waitForUtilities();
      imageDetector = new ImageDetector('discord');
      messageAnalyzer = new MessageAnalyzer();
      await messageAnalyzer.initialize();
      setupMessageMonitoring();
      setupGhostHubButton();
      isInitialized = true;
      console.log('[GhostHub] Discord integration initialized');
    } catch (error) {
  ...
  function setupMessageMonitoring() {
    ...
    if (imageDetector) {
      imageDetector.observeImages(chatContainer, (images) => {
        console.log(`[GhostHub] Detected ${images.length} new image(s) in Discord`);
        images.forEach(img => {
          if (img.element) {
  ...
  function getRelevantMessages() {
    ...
    messageElements.forEach((msgEl, index) => {
      const textElement = msgEl.querySelector('[class*="messageContent"]');
